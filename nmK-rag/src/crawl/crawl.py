...
def clean_state():
    ...
    try:
        os.remove(DB_PATH)
        logger.info("이전 방문 기록 DB 삭제됨")
        print("✅ 이전 방문 기록 삭제 완료")
    except FileNotFoundError:
        pass
    try:
        os.remove(STATE_FILE)
        logger.info("이전 상태 파일 삭제됨")
        print("✅ 이전 상태 파일 삭제 완료")
    except FileNotFoundError:
        pass
    logger.info("상태 초기화 완료. 완전히 새로운 크롤링을 시작합니다.")
...
if __name__ == "__main__":
    try:
        ...
        if choice == '1':
            print("📋 일반 실행 모드를 시작합니다...")
        elif choice == '2':
            print("🆕 완전히 새로 시작합니다...")
            fresh = True
            clean_state()
        elif choice == '3':
            print("🔄 모든 페이지를 강제 재크롤링합니다...")
            force = True
        elif choice == '4':
            print("💬 대화형 모드를 시작합니다...")
            interactive = True
        elif choice == '5':
            print("🆕💬 새로 시작 + 대화형 모드를 시작합니다...")
            fresh = True
            interactive = True
            clean_state()
