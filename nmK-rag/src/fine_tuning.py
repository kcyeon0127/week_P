...
정확하고 전문적인 정보를 제공하며, 교육적 가치가 있는 설명을 해줘."""
    ...
    def train_model(self, train_dataset: Dataset, eval_dataset: Dataset, output_dir: str, target_type: str):
        ...
        training_args = TrainingArguments(
            ...
            num_train_epochs=5,                    # 3 → 5 (더 충분한 학습)
            per_device_train_batch_size=4,        # 2 → 4 (Tesla V100 최적화)
            gradient_accumulation_steps=2,        # 4 → 2 (실제 배치 8개)
            warmup_steps=50,                      # 100 → 50 (데이터 크기에 맞춤)
            learning_rate=3e-4,                   # 2e-4 → 3e-4 (조금 더 빠른 학습)
            logging_steps=5,                      # 10 → 5 (더 자주 로깅)
            logging_dir=f"{output_dir}/logs",
            save_strategy="epoch",
            evaluation_strategy="epoch",          # no → epoch (성능 모니터링)
            eval_steps=None,
            load_best_model_at_end=True,          # False → True (최고 성능 모델 저장)
